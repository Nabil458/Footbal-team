...
function testmatch(): void
{
    try{
    $this->repository->fillDatabase();
    foreach ($this->data->matches() as $matche) {
        $this->assertEquals($this->repository->team($matche['id']), $matche);
    }
    }catch(Exception $exception){
        echo($exception->getMessage());
    }
}
/*function testMatchesThrowsExceptionIfMatchesDoesNotExist(): void
{
    $this->repository->fillDatabase();
    $this->expectException(Exception::class);
    $this->expectExceptionMessage('Équipe inconnue');
    $this->repository->matches(10000);
}*/
